...
public class EnemyAIBase : MonoBehaviour
{
    ...
    private void Awake()
    {
        target = GameObject.Find("Player").transform; //Busca al jugador por su nombre y obtiene su transform
        agent = GetComponent<NavMeshAgent>(); //Obtiene el componente NavMeshAgent del enemigo para controlarlo
        lastPosition = transform.position; //Inicializa la posición del último punto perseguido con la posición actual del enemigo
        lastCheckTime = Time.time; //Inicializa el tiempo del último chequeo de stuck con el tiempo actual del juego
    }
